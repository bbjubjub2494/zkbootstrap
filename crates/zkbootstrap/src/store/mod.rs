...
pub trait Backend<'a> {
    fn add_node(&mut self, program: impl Into<BlobOrOutputRef>, input: impl Into<BlobOrOutputRef>, name: String) -> NodeRef;
...
impl <'a, B: Backend<'a>> Store<B> {
    ...
    pub fn reexecute(
        ...
        let output_ref = self.add_blob(output_bytes.into(), node.output_name.clone());
    ...
    pub fn add_node(&mut self, program: impl Into<BlobOrOutputRef>, input: impl Into<BlobOrOutputRef>, name: String) -> NodeRef {
        self.backend.add_node(program, input, name)
    }
    pub fn add_blob(&mut self, bytes: Cow<'a, [u8]>, name: String) -> BlobRef {
        self.backend.add_blob(bytes, name)
    }
    ...
    fn inner_walk(&self, r: &BlobOrOutputRef, f: &mut impl FnMut(&Self, WalkStep) -> Result<()>) -> Result<()> {
        match r {
            ...
            BlobOrOutputRef::BlobRef(blob_ref) => {
                f(self, WalkStep::Blob(blob_ref.clone()))?;
            }
        }
        Ok(())
    }
